...
export function PublicPageRenderer({ layout, data = {}, bindings, isLoop }: PublicPageRendererProps) {
      ...
      case "RichText":
        return (
          <div 
            key={block.id} 
            style={styleString} 
            className="prose prose-blue max-w-none my-6 text-gray-800 leading-relaxed"
      ...
      case "Collection List":
        ...
        return (
          <div key={block.id} style={styleString} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 my-8">
            {posts.length > 0 ? (
              posts.map((post: any) => (
                <Link href={`/blog/${post.slug || post.id}`} key={post.id} className="group block border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition bg-white flex flex-col h-full">
                  <div className="relative w-full h-48 bg-gray-100">
                    <Image src={post.coverImage || post.imageUrl || post.featured_image || "https://placehold.co/400x300"} fill className="object-cover group-hover:scale-105 transition-transform duration-300" alt={post.title} />
                  </div>
